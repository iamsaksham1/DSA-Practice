class Solution {
 public:
  string smallestNumber(string num, long long t) {
    ...
    // No solution of the same length exists, so extend the number by
    // prepending '1's and adding the required factors.
 ...
 private:
  ...
  // Returns the prime factorization of t (over {2,3,5,7}) and whether t has
  // no other prime factors (i.e. is achievable by digit products at all).
  ...
  // Returns the combined prime factor count of all digits in `num`.
  ...
  // Greedily packs the required prime factors (count[2], count[3], count[5],
  // count[7]) into the fewest possible digits (preferring larger digits like
  // 8, 9 over multiple smaller ones), which minimizes how many extra digits
  // we need to append.
  unordered_map<int, int> getFactorCount(const unordered_map<int, int>& count) {
    ...
    // Combine leftover 2 and 3 into a single 6 if both are present.
    int count6 = 0;
    ...
    // Combine leftover 3 and 4 into 2 and 6 if both are present.
    ...
    return unordered_map<int, int>{
        {2, count2}, {3, count3},
        {4, count4}, {5, count.at(5)},
        {6, count6}, {7, count.at(7)}, {8, count8}, {9, count9}};
  }

  // Builds the smallest string (digits in ascending order) representing the
  // given multiset of required trailing digits.
  ...
  // Returns true if every prime count in `a` is <= the corresponding count in `b`.
  ...
  // Returns a - b, clamped at 0 per key.
